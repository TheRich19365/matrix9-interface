...
export function buildPosterPrompt({ analysis, reading, includeSacredResonance = false }) {
  ...
  return [
    ...
    `Primary Evidence: ${primaryBeing.evidence?.join(" | ") || "-"}`,
    `Supporting Star Resonance: ${supportingBeing.name} — ${supportingBeing.title}`,
    `Supporting Visual Language: ${supportingBeing.keywords.slice(0, 4).join(", ")}`,
    `Supporting Evidence: ${supportingBeing.evidence?.join(" | ") || "-"}`,
    `Earth Integration Path: Gate ${earthIntegration.gate || "-"} — ${earthIntegration.title}`,
    `Earth Integration Symbolism: ${earthIntegration.shortMeaning || earthIntegration.meaning}`,
    `Earth Integration Evidence: ${earthIntegration.evidence?.join(" | ") || "-"}`,
    "",
    ...
    "Use Supporting Star Resonance only as secondary color, geometry, or atmosphere. Do not replace the Primary Core Energy.",
    "Show Earth Integration Path as grounded environmental/action symbolism, not as a second starseed identity.",
